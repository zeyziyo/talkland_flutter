...
class SentenceRepository {
  ...
  static Future<int> insert(Map<String, dynamic> data, {Transaction? txn}) async {
    ...
    return await executor.insert('sentences', data, conflictAlgorithm: ConflictAlgorithm.ignore);
  }

  static Future<List<Map<String, dynamic>>> getSentencesByGroupId(int groupId) async {
    final db = await _db;
    return await db.query('sentences', where: 'group_id = ?', whereArgs: [groupId]);
  }
  ...
  static Future<List<Map<String, dynamic>>> searchAutocompleteText(String langCode, String text) async {
    ...
    return await db.query(
      'sentences',
      columns: ['id', 'text', 'group_id', 'form_type', 'style', 'note'],
  ...
  static Future<void> updateMemorizedStatus(int id, bool status) async {
    final db = await _db;
    await db.update('sentences', {'is_memorized': status ? 1 : 0}, where: 'id = ?', whereArgs: [id]);
  }
